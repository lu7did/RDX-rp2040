...
void quitProc() {
  ...
  _INFO("Client disconnected by user\n");
  TCPclient.stop();
  TCPconnected=false;
  return;
...
/*----------------------------------------------------------------------------------------------*/
  ...
  quitCallBack=quitProc;
  TCPserver.begin();
  cli_init(TCPout);
  TCPconnected=false;
  _INFO(" Server initialized(%s)\n",TCPout);
...
int clientWrite(WiFiClient c, char *s) {
...
void TCP_Process() {
  ...
  if (!TCPclient.available()) {
     if (TCPclient.status() != 4) {
        _INFO("Client status=%d terminated by remote\n",TCPclient.status());
        TCPclient.stop();
        return;     
     }
  }
  ...
  while (TCPclient.available()) {
     TCPChar=TCPclient.read();
     if (TCPChar != '\r' && TCPChar != '\n') {
        TCPinBuffer[TCPptrBuffer++]=TCPChar;
        TCPinBuffer[TCPptrBuffer]=0x00;       
     } else {
        if (TCPChar == '\n') {
           wipeChar(TCPinBuffer);
           if (strcmp(TCPinBuffer,"q")==0) {
              if (quitCallBack!=NULL) quitCallBack();
              return;
           }         
           bool rc=cli_execute(TCPinBuffer,TCPoutBuffer);
           delay(200);
           TCPclient.printf(TCPoutBuffer);
           TCPptrBuffer=0;
           strcpy(TCPinBuffer,"");
           strcpy(TCPoutBuffer,"");
           return;
